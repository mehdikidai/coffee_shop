<x-layout title="categories Page" name_page="page-categories">
    ...
    <x-only-admin>
        <div class="btns">
            <button type="button"
                class="k-button-add-category btn btn-primary mb-3 float-end btn-sm d-flex align-items-center px-3"
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5 text-capitalize" id="staticBackdropLabel">
                        {{ __('t.add_new_category') ?? "add new category" }}
                    </h1>
                    ...
                    <form action="{{ route('categories.store') }}" method="post">
                        ...
                        <div class="mb-3 input-group-sm">
                            <label for="exampleFormControlInput1" class="form-label text-capitalize">
                                {{ __('t.name_category') ?? "name category" }} </label>
                            <input type="text" name="name" class="form-control"
                                placeholder="{{ __('t.name_category') ?? "name category" }}">
                        </div>
                        <div class="d-grid mt-4 input-group-sm">
                            <button type="submit" class="btn btn-primary text-capitalize">
                                {{ __('t.add_category') ?? "add category" }} </button>
                        </div>
